...
def get_end_value(data):
	return int(data.split(';')[1].split('=')[1])


def resolve_row(chromosome_type, data_row, reference_row, min_range, max_range):
	if str(data_row[chromosome_type]) == str(reference_row['CHROM'])[3:]:
		if (compare_min_max_values(int(data_row['PosFrom']), int(reference_row['POS']), get_end_value(reference_row['INFO']), min_range, max_range)) or \
				(compare_min_max_values(int(data_row['PosTo']), int(reference_row['POS']), get_end_value(reference_row['INFO']), min_range, max_range)):
			return True
